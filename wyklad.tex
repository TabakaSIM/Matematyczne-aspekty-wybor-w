...
\end{exercise}
	...
	\begin{example}
		\begin{enumerate}[1.]
			...
			\item Metoda Coombsa (Clyde Coombs, 1912–1988, USA, psycholog) – Wypisujemy kolejność wyników głosowania, odrzucamy kandydata, który ma najwięcej ostatnich miejsc, i głosujemy dalej, aż ktoś uzyska ponad $50\%$ pierwszych miejsc. Gdy nie można nikogo odrzucić, wygrywają wszyscy, którzy pozostali.
	...
	\end{example}
		...
		\begin{tabular}{|p{2.5cm}|p{2cm}|p{2cm}|p{2cm}|p{2cm}|c|p{2cm}|}\hline
			Metoda & Kryrterium Condorceta & Kryrterium przegranego Condorceta & słaba zasada Pareto &Monoto- niczność ze względu na transpozycję &IIA&Jedno- znaczna większość\\\hline
			Punkty Bordy   &-(1)&+&+&+&-(2)&-(1)\\\hline
			Hare'a   &-(3)&-(3)&+&-(4)&-(5)&+(O)\\\hline
			Coombsa &&&+&-(13)&-(13)&+(O)\\\hline
		...
		\end{tabular}\\\\
		\textbf{Metoda Bordy, K przegranego Condorseta}
		$\forall_w A<B$ Punkty $A$ < Punkty $B$
		w punktuje A dostaje 1 punkt za każdą parę $(B,w)$w którego w stawia B ponad A
		
		A - przegrany condorseta
		k kandydatów, n wyborców
		$K_1,...,K_{n-1}$ u każdego będzie miał $A<\frac{n}{2}$ punktów. A w sumie ma mniej niż $\frac{k(k-1)}{2}$ punktów
		
		A - B punktów do rozdziału (w parze z każdym kandydatem dostaje mniej punktów)
		
		W sumie do rozdziału jest $n\cdot\frac{k(k-1)}{2}$ punktów (suma ciągu arytmetycznego). Wszyscy wobec tego mają mniej niż $\frac{n(k-1)}{2}$ punktów. W sumie jest $<\frac{k\cdot n \cdot (k-1)}{2}$
		
		\textbf{M COmbsa i Hare'a}
		$\forall_w A\overset{w}{<}B$ to A wczesniej odpadnie od B
		\textbf{IIA dyktatura}
		A wygrywa, $B\overset{d,M}{<}A$ B nie może wygrać
	
		\textbf{Metoda terminarzowa - k condorseta}
		
		Kandydat condorseta wygrywa z każdym i nie odda zwycięstwa	
	
		\textbf{Terminażowa - monotoniczność} po przesunięciu też wygrywa 
	
		\textbf{Metod copelanda} Jak z każdym wygrywa to ma max punktów
		Zasada pareto - $\forall_w B\overset{w}{<}A \overset{?}{\Rightarrow}B nie wygrywa$
		
		B wygrywa w pojedynku z C $\Rightarrow$ A wygrywa w pojedynku z C. Wobec tego punkty A $>$ punkty B, bo A dostaje punkty za parę $(A-B)$. Wobec tego punkty A są silniejsze.
	
		\textbf{Metoda Coombsa - słaba zasada Pareto}
		$\forall_w B\overset{w}{<}A$ - A nie odpadnie zanim B nie odpadnie (B odpadnie wcześniej niż A). 

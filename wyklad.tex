...
\begin{wniosek}[Twierdzenie Arrowa o niemożliwości]
	Załóżmy, że $\# K \geq 3$, $\# W \geq 2$. Wówczas: nie istnieje metoda zbiorowego ustalania (MZU) efektywna, która jednocześnie spełnia następujące warunki:
	\begin{enumerate}[-]
		\item anonimowość,
		\item słabą zasadę Pareto,
		\item słabe kryterium niezależności od alternatyw niezwiązanych (IIA).
	\end{enumerate}
\end{wniosek}
...
\begin{theorem}[Twierdzenie Taylora o niemożliwości]
	Dla $\# K \geq 3$, $\# W \geq 3$:  
	nie istnieje MZU efektywna, która spełnia jednocześnie kryterium Condorceta oraz słabe kryterium niezależności od alternatyw niezwiązanych (IIA).
\end{theorem}
...
\begin{example}[Szczególny przypadek metody status quo]
	Przypadek z dwoma kandydatami. Założenia:
	\begin{itemize}
...
\begin{definition}[Wyborca decydujący]
	Załóżmy, że $A$ jest koalicją rządzącą.  

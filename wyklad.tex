...
Załóżmy teraz, że partia \textbf{Rybacy} prowadziła kampanię przeciwko \textbf{Myśliwym}, w wyniku czego liczba otrzymanych głosów zmieniła się następująco:
\begin{itemize}
...
\begin{tabular}{|c|c|c|c|c|c|}\hline
	Nazwa    & Głosy $:1$ & $:2$ & $:3$ & $:4$ & Mandaty\\\hline
	Rybacy   & \textbf{6400} & \textbf{3200} & 2133 &       & 2\\\hline
	Myśliwi  & \textbf{5100} & \textbf{2550} & 1700 &       & 2\\\hline
	Artyści  & \textbf{2150} & 1075          &      &       & 1\\\hline
\end{tabular}\\

\Komentarz{Tym oto sposobem partia \textbf{Myśliwi} stracił§a jeden mandat na rzecz partii \textbf{Artyści}.}
...
\begin{tabular}{|c|c|c|c|c|c|c|}\hline
	...
	Kelnerzy     & \textbf{1050} & \textbf{350} & \textbf{210} & \textbf{150} & 117  & 4\\\hline
	Sportowcy    & \textbf{1008} & \textbf{336} & \textbf{202}  & 144 &      & 3\\\hline
\end{tabular}\\

Na ten moment wygrywa partia \textbf{Kelnerzy} 4 mandatami. Partia \textbf{Sportowcy} postanowiła się rozdzielić na dwie partie i startować osobno:
...
\\ $\textbf{W}$ - zbiór wszystkich wyborców, \\
$\textbf{K}$ - zbiór wszystkich kandydatów.
\begin{itemize}
...
\begin{definition}[Metoda anonimowa]
	Metoda jest anonimowa \witw gdy wszyscy wyborcy są traktowani tak samo, to znaczy \begin{center}
		$\forall_{x,y \in W}$ zamiana głosów $x$ i $y$ nie zmienia wyniku.
	\end{center}
\end{definition}

\Komentarz{Alternatywnie, metoda nie jest anonimowa kiedy \begin{center}
		 $\exists_{x,y \in W}$ takie, że zamiana głosów $x$ i $y$ \textit{istotnie} zmienia wynik.
\end{center}}

\begin{definition}[Metoda neutralna]
	Metoda jest neutralna gdy wszyscy kandydaci są traktowani tak samo, to znaczy \begin{center}
		$\forall_{x,y \in K}$ zamiana ról $x$ i $y$ nie zmienia wyniku.
	\end{center}
\end{definition}

\Komentarz{Alternatywnie, metoda nie jest neutralna  \begin{center}$\exists_{x,y \in K}$ takie, że zamiana ról $x$ i $y$ \textit{istotnie} zmienia wynik,\end{center} dokładniej definiując:\\\\
...
\newpage
	...
	\begin{definition}[Metoda zakładająca uporządkowanie]
		Metoda zwycięzcy jest metodą zakładającą uporządkowanie (MZU) gdy \begin{center}
	...
	\end{example}
		...
		\begin{definition}[Słaba zasada Pareto]
			MZU spełnia słabą zasadę Pareto gdy \begin{center}
				$\forall_M (\exists_{A,B\in K}\forall_{w\in W} A\overset{w,M}{<}B) \Rightarrow A$ nie wygrywa w $M$.
			\end{center}
		\end{definition}
		
		\begin{definition}[Kandydat Condorceta]
			$A\in K$ jest kandydatem Condorceta (zwycięzcą Condorceta) gdy w "bezpośrednich porównaniach" $A$ jest lepszy od każdego innego kandydata czyli \begin{center}
				$\forall_{B \in K}: B\neq A \quad \#\{w: B\overset{w,M}{<}A\}>\#\{w: B\overset{w,M}{>}A\}$.
			\end{center}
		\end{definition}
		
		\begin{definition}[Przegrany Condorceta]
			$A\in K$ to przegrany Condorceta (w modelu $M$) gdy \begin{center}
				$\forall_{B \in K}: B\neq A \quad\#\{w: B\overset{w,M}{<}A\}<\#\{w: B\overset{w,M}{>}A\}$.
			\end{center}
		\end{definition}
		
		\begin{definition}[Kryterium Condorceta]
			Metoda spełnia kryterium Condorceta gdy \begin{center}
				$\forall_M$ istnieje kandydat Condorceta (w $M$) $\Rightarrow A$ - jedyny zwycięzca w $M$.
			\end{center}
		\end{definition}
		
		\begin{definition}[Kryterium przegranych Condorceta]			
			Metoda spełnia kryterium przegranych Condorceta gdy \begin{center}
				$\forall_M$ istnieje przegrany Condorceta $A$ w $M$ $\Rightarrow A$ nie wygrywa w $M$.
			\end{center}
		\end{definition}
		
		\begin{definition}[Metoda jednoznacznie większościowa]
			Metoda jest jednoznacznie większościowa gdy \begin{center}
				$\forall_M$ kandydat $A$ w $M$ ma ponad połowę pierwszych miejsc $\Rightarrow A$ - jedyny zwycięzca.
			\end{center}
		\end{definition}
		...
		\end{theorem}
		\noindent \textbf{Dowód:} Mamy dany model $M$ w którym $\forall_{w\in W} A\overset{w,M}{<}B \overset{?}{\Rightarrow}$ $A$ nie wygrywa w $M$.\\\\
		...
		\begin{theorem}
			MZU, monotoniczna ze względu na transpozycje \witw($\forall_{M - \text{model}} \forall_{A\in K} \forall_{B_1,\dots,B_n\in A} (A\neq B, \text{ ale może być } B_i=B_j) \forall_{w_1,\dots,w_n\in W}$ (może być $w_i=w_j$), jeżeli w modelu $M$ wygrywa $A$, to model $N$ utworzony przez $[\Delta, B_i, A, *] \rightarrow [\Delta, A, B_i, *]$ dla $i=1,\dots,n$ $\Rightarrow$ w $N$ dalej wygrywa $A$.)
		\end{theorem}
		...
		\subsection{Własności metod wyborczych}
...
\end{definition}
	...
	\begin{definition}[Warunek (DEC)]
		MZU
		\begin{center}
			 $\forall_M (\forall_{x \in W} \; a \overset{x, M}{<} b) \Rightarrow a \underset{M}{<} b$  
		$(I)$ $\forall_{M, N} \forall_{x \in W} \; (a \overset{x, M}{<} b \iff a \overset{x, N}{<} b)$
		\end{center}

...
\end{definition}
	...
	\begin{definition}[Liberalizm Senna]
		$\forall_{w\in W} \exists_{a,b \in K; a\neq b }\forall_M w decyduje o (a,b)$
	\end{definition}
	
	\begin{theorem}
		Dla K i W istnieje metoda spełniająca postulat liberalizmu Senna \witw $\# W < \# K$
	\end{theorem}
	
	D: $(\Leftarrow)$ $K=\{k_1,\dots, k_n\} W=\{w_1,\dots,w_s\} \: s\leq k-1$
	$w_1:k_1,k_n$
	$w_2:k_2,k_n$
	\vdots
	$w_s:k_s,k_n$
	
	Metoda $k_n $ o $k_1,\dots, k_s$ w stosunku do $k_n$ decydują $w_1, w_s$ potem ustawiamy numerami rosnąco:
	...
	$(\Rightarrow)$ TODO2 dowód graficzny
	
	Wprowadźmy zonaczenia:\\
	$\cdot - k_i$\\
	$| - w_i$ łączy tych kandydatów, o których decyduje
	
	Hp. $\# "|" > \# "\cdot"$
	
	\begin{enumerate}[1.]
		\item Rysujemy ów graf
		\item Odrzucamy kropki bez kreski oraz z jedną kreską
		Zostaje $n$ kropek i $l$ kresek, przy czym $(l\geq n)$.
		...
		\end{bmatrix}$, a z tego $\begin{bmatrix}
		k_1\overset{w_1}{<}k_2\\
		k_2\overset{w_2}{<}k_3\\
		\vdots\\
		k_s\overset{w_s}{<}k_1\\
	...
	$m\cdot\frac{p_i}{p}(\text{zazwyczaj }\notin \mathbf{Z})$ - to, co powinien dostać\\
	
	$W=\frac{p}{m}$ - wartość akcji (jednej)\\
	
	$q_1:= m\cdot \frac{p_1}{p}=\frac{p_1}{\frac{p}{m}}=\frac{p_1}{w}$ - quota\\
	
	$\lfloor q_1 \rfloor=\lfloor \frac{m\cdot p_i}{p} \rfloor$ - dolna quota\\
	$\lceil q_1 \rceil=\lceil \frac{m\cdot p_i}{p} \rceil$ - górna quota
	\end{definition}
	
	\begin{definition}[warunki sensowności metody rozdziału]
		\begin{enumerate}[1.]
			\item (warunek quoty) $\lfloor q_i \rfloor \leq a_i \leq \lceil q_i \rceil$ (uwzględniająć  $q_i \in \mathbf{Z} \Rightarrow g_i = a_i$)
			\item (warunek monotoniczności) $p_i>p_j \Rightarrow a_i \geq a_j$ (analogicznie w drugą stronę)
			\item (warunek populacji) S,m dane; $p_1,\dots,p_s \longmapsto a_1,\dots,a_s$
			nastąpiła zmiana
			$\bar{p_1},\dots,\bar{p_s}\longrightarrow \bar{a_1},\dots,\bar{a_s}$
			
			$!\exists_{i,j} \bar{p_i}> p_j, \bar{a_i}<a_i$ oraz $\bar{p_j}<p_j, \bar{a_j}>a_j$
			\item (warunek monotoniczności akcji) $p_1,\dots,p_n$ stałe ,$\bar{m}>m \Rightarrow \forall_i \bar{a_i}\leq a_i$
		\end{enumerate}
	...
	\begin{example}
		\begin{enumerate}[1)]
			\item Metoda reszt (meroda Hamiltona)
			$a_i=\lfloor q_i \rfloor + \epsilon_i$, gdzie $\epsilon_i=0$ lub $1$, bierzemy największe reszty $q_i-\lfloor a_1 \rfloor$ aż do "wyczerpania" m\\\\
			
			$S=3,\: m=10, \: W=\frac{1000}{10}=100$\\
			\begin{tabular}{|c|c|c|c|c|c|}\hline
				&$p_i$&$q_i$&$\lfloor q_i \rfloor$&$r_i$& Wynik\\\hline
			...
			Niech $S=3, \: m=10, \: W=100$\\
			\begin{tabular}{|c|c|c|c|}\hline
			$p_i$&$q_i$&$\lfloor q_i \rfloor$& Wynik\\\hline
			145&1,45&1&2\\
			340&3,40&3&3\\
			515&5,15&5&5\\\hline
			...
			Zmieniamy liczbę akcji $m=11,\: W=90.,9$\\
			\begin{tabular}{|c|c|c|c|}\hline
				$p_i$&$q_i$&$\lfloor q_i \rfloor$& Wynik\\\hline
				145&1,595&1&1\\
				340&3,740&3&4\\
			...
			Zmieniami liczbę acjonariuszy i akcji $S=4,\: m=13, \: W=96,9$
			...
			\begin{tabular}{|c|c|c|c|}\hline
				...
				515&5,31&5&5\\
				200&2,68&2&3\\\hline
				$\Sigma=1000$&&11&13\\\hline	
			\end{tabular}
			
			
			\item Metoda Jeffersona
			...
			\begin{tabular}{|c|c|c|c|}\hline
				$p_i$&$q_i$&$\bar{q_i}$& $\lfloor \bar{q_i} \rfloor$\\\hline
				264&2,64&1&2\\
				361&3,61&3&3\\
				375&3,75&5&5\\\hline
				$\Sigma=1000$&&9&10\\\hline	
			...
			\item Przybliżanie do wartościami umownymi do górnych quot (Metoda Adamsa)\\
			
			Niech $S=2,\: m= 10,\: W=100, \:v=115$\\
			\begin{tabular}{|c|c|c|c|c|c|}\hline
				&$p_i$&$q_i$&&$\bar{q_i}$& Wynik\\\hline
				A&120&1,8&2&1,04&2\\
				B&880&8,8&9&7,65&8\\\hline
				&$\Sigma=1000$&&11&&10\\\hline
			\end{tabular}
			
			\item Zaokrąglenie do bliższej dolnej/górnej (Metoda Webstera)\\
			Niech $S=3,\: m=5$\\
			\begin{tabular}{|c|c|c|c|}\hline
				&$p_i$&$q_i$& Wynik\\\hline
				A&480&2,66&3\\
			...
			\item Metoda Deana (to samo, ale ze średnią harmoniczną)
